...
// here we took assumption that the cacheLineSize mus be greater than 4, for our
// request comes in 4 bytes currently we took "unaligned access" out of
// consideration
SC_MODULE(CACHEL2) {
  int latency;
  ...
  // we can possibly divide these bits into : Tag bits|Data bits|Control
  // bits(dirty)
  CacheLine* internal;
    ...
    if (index == -1) {  // in the cache there are no such information
      ...
      for (int i = 0; i < (int)(cacheLineSize / 4); i++) {
        ...
        std::cout << name << " sended addr: " << std::hex << std::setw(8)
                  << std::setfill('0') << addressToLastStage.read().to_int()
                  << " with original addr: " << std::hex << std::setw(8)
