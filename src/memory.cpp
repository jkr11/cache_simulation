...
SC_MODULE(MEMORY) {  // due to the defination of Request, I assume that the max
                     // address and maximal length of Data are 4 Bytes (uint32)
  int latency;
  sc_in<bool> clk;
  ...
  std::unordered_map<uint32_t, uint8_t>
      internal;  // this unordered_map allows the small range of simulation of
                 // 4GB memory in PC
  ...
  void run() {
    while (true) {
      ...
      if (requestIncoming.read()) {
        ready.write(false);
        /*while (latencyWaited < latency){ // first count the latency then
        execute the funtionality
        ...
        if (rw.read()) {  // true for write
          std::cout << "memory recieved data in binary: "
                    << rData.read().to_string() << std::endl;
          for (int i = 0; i < 4; i++) {
            internal[address + i] =
                rData.read().range((i + 1) * 8 - 1, i * 8).to_uint();
            std::cout << "written into address: " << address + i
                      << " with data: " << std::hex << std::setw(1)
                      << std::setfill('0')
                      << rData.read().range((i + 1) * 8 - 1, i * 8).to_uint()
                      << std::endl;
          }
          ...
          for (int i = 0; i < 4; i++) {
            if (internal.find(address + i) == internal.end()) {
              ...
              std::cout << "read from address: " << address + i
                        << " with data: 0x00" << std::endl;
            } else {
              data.range((i + 1) * 8 - 1, i * 8) = internal[address + i];
              std::cout << "read from address: " << address + i
                        << " with data: " << std::hex << std::setw(2)
                        << std::setfill('0') << (int)internal[address + i]
                        << std::endl;
            }
          }
          wData.write(data);
