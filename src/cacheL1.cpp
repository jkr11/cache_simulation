...
SC_MODULE(CACHEL1) {
  ...
  CacheLine*
      l2;  // the internal storage of l2, for a faster communication with l2
  ...
  sc_in<sc_bv<32>>
      dataFromLastStage;  // Wichtig: for a better communication, this will be
                          // the index of the array in l2 storage
  ...
  sc_out<bool> isWriteThrough;  // the signal to l2, shows that if the current
                                // write Request a Write Through is
        ...
        */
        if (rw.read()) {  // write enable
          std::cout << name << " received <write> with addr:" << std::hex
                    ...
                    << address.read().to_int()
                    << " and data:" << inputData.read().to_int() << std::endl;
          write();
        } else {
  ...
  void write() {
    bool hit = true;
    int index = -1;
    // check if the input address has different index (the lowest address, the
    // highest address)
    ...
    if (offset_tmp <
        cacheLineSize -
            3) {  // the accessed 4 Byte can be found in one Cache line
      std::cout << "start dealing data that can be extracted from one line"
                << std::endl;
      ...
      // //2.0 We dont need this alignment anymore
      for (int i = 0; i < 4; i++) {
        internal[index % cacheLines].bytes[offset + i] =
            inputData.read()
      ...
      std::cout << "start dealing data that can be extracted from one line "
                ...
                << addressBV_low.to_uint() << " and highest: " << std::hex
                << std::setw(8) << std::setfill('0') << addressBV_high.to_uint()
                << std::endl;
      ...
      if (index == -1) {  // in the cache there are no such information
        std::cout << name << " miss by writing with addr: " << std::hex
                  ...
                  << address.read().to_int()
                  << " detected, sending signal to next level" << std::endl;
        hit = false;  // if the first accessed cache line is missed, then shall
                      // this eventually be a miss
        index = loadFromL2(address.read().to_uint());
      }
      if (hit) {
        std::cout << name << " hit with addr: " << std::hex << std::setw(8)
      ...
      // offset = offset & 0xfffffffc; // the address is always 4 Byte aligned
      // //2.0 We dont need this alignment anymore
      int i;  // the number to record the written Bytes in the first line, so
              // that we can continue to write the rest in the second
      for (i = 0; i + offset < cacheLineSize; i++) {
        internal[index % cacheLines].bytes[offset + i] =
            inputData.read().range(31 - i * 8, 31 - (i + 1) * 8 + 1).to_uint();
      }
      ...
      requestToLastStage.write(false);
      std::cout << name
                << " finished with loading the first line from l2 with addr: "
                << std::hex << std::setw(8) << std::setfill('0')
                << address.read().to_int() << std::endl;
      ...
      // at this point, the required data is successfully loaded from last stage
      // calculate the offset and change the required data
      // int offset = address.read().range(indexOffset-1,0).to_uint(); we dont
      // need a offset here because the byte shall always be written from the
      // beginning in the second line offset = offset & 0xfffffffc; // the
      // address is always 4 Byte aligned //2.0 We dont need this alignment
      // anymore
      for (int j = i; j < 4; j++) {
        internal[index % cacheLines].bytes[j - i] =
            inputData.read().range(31 - j * 8, 31 - (j + 1) * 8 + 1).to_uint();
      }
      printCacheLine(index % cacheLines);
      ...
      ready.write(true);
      std::cout << name
                << " finished with loading the second line from l2 with addr: "
                << std::hex << std::setw(8) << std::setfill('0')
                << address.read().to_int() << std::endl;
    }
  }
  void read() {
    ...
    if (offset_tmp <
        cacheLineSize - 3) {  // the read data can be accessed in one cache line
      int t_tmp = address.read().range(31, tagOffset).to_uint();
      int i_tmp = address.read().range(tagOffset - 1, indexOffset).to_uint();
      index = ifExist(t_tmp, i_tmp);
      if (index == -1) {  // in the cache there are no such information
      ...
      sc_bv<32> output_tmp;
      for (int i = 0; i < 4; i++) {
        output_tmp.range(31 - i * 8, 31 - (i + 1) * 8 + 1) =
            internal[index % cacheLines].bytes[offset + i];
      }
      outputData.write(output_tmp);  // here we are able to check if the read
                                     // information is as expected
      ready.write(true);
      std::cout << name << " finished with addr: " << std::hex << std::setw(8)
                << std::setfill('0') << address.read().to_int()
                << " with read data:" << output_tmp.to_int() << std::endl;
      ...
      for (i = 0; offset + i < cacheLineSize; i++) {
        output_tmp.range(31 - i * 8, 31 - (i + 1) * 8 + 1) =
            internal[index % cacheLines].bytes[offset + i];
        std::cout << name << " current loaded: " << std::hex << std::setw(8)
                  << std::setfill('0') << output_tmp.to_int() << std::endl;
      }
      ...
      printCacheLine(index % cacheLines);
      for (int j = i; j < 4; j++) {
        output_tmp.range(31 - j * 8, 31 - (j + 1) * 8 + 1) =
            internal[index % cacheLines]
                .bytes[j - i];  // read from the beginning of the second line
        std::cout << name << " current loaded: " << std::hex << std::setw(8)
                  << std::setfill('0') << output_tmp.to_int() << std::endl;
      }
  ...
  void writeThrough(int index, int addressToWrite) {
    ...
    // by write Throught, we provide a index for l2 to gain immediate access to
    // the whole cache line of l1
